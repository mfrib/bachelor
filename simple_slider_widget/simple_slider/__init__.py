...
class Widget():
    ...
    def __init__(self, data_dir=None, num_planets=None):
        ...
        self.r     = (np.loadtxt(os.path.join(data_dir, 'radius.dat'))/au)
        ...
        """
        #useful variable
        r_max = np.log10(self.r[-1])
        r_min = np.log10(self.r[0])
        ...
        self.model_line_T, = self.ax.loglog(self.r, self.sigma[:, int(self._slider_T.val)], 'k-')
        ...
        self._slider_Norm = w.Slider(self._ax_Norm, "$y_0$", 0, 5, valinit = 2.04, valfmt = '%.2f')
        ...
        self.model_line_A, = self.ax.loglog(self.r, np.ones_like(self.r))
        ...
        if self.planet[0] == 1:
            ...
            self.marker_gauss1, = self.ax.loglog(self.r[100],self.sigma[-1,0], marker='o', linestyle='None', markersize=10, color="salmon", markeredgecolor='black')
        
        # Gaussian slider 2
        if self.planet[1] == 1:
            self._ax_g2x = self.fig.add_axes([g2x_slider_x, g2x_slider_y, g2x_slider_w, g2x_slider_h], facecolor="turquoise")
            self._slider_g2x = w.Slider(self._ax_g2x, "$x_0$", r_min, r_max, valinit = r_min, valfmt = '%.2f')
            self._ax_g2x.set_title("Gaussian position", fontsize='small')
            self._slider_g2x.on_changed(self.update)       
            
            self._ax_g2w = self.fig.add_axes([g2w_slider_x, g2w_slider_y, g2w_slider_w, g2w_slider_h], facecolor="turquoise")
            self._slider_g2w = w.Slider(self._ax_g2w, "$w$", 0, 1.5, valinit = 0, valfmt = '%.2f')
            self._ax_g2w.set_title("Gaussian width", fontsize='small')
            self._slider_g2w.on_changed(self.update)
            
            self._ax_g2A = self.fig.add_axes([g2A_slider_x, g2A_slider_y, g2A_slider_w, g2A_slider_h], facecolor="turquoise")
            self._slider_g2A = w.Slider(self._ax_g2A, "$A$", 0, 1, valinit = 0, valfmt = '%.2f')
            self._ax_g2A.set_title("Gaussian amplitude", fontsize='small')
            self._slider_g2A.on_changed(self.update)
            self.marker_gauss2, = self.ax.loglog(self.r[0],self.sigma[-1,0], marker='o', linestyle='None', markersize=10, color="turquoise", markeredgecolor='black')
        
        #Gaussian sliders 3
        if self.planet[2] == 1:
            self._ax_g3x = self.fig.add_axes([g3x_slider_x, g3x_slider_y, g3x_slider_w, g3x_slider_h], facecolor="khaki")
            self._slider_g3x = w.Slider(self._ax_g3x, "$x_0$", r_min, r_max, valinit = r_min, valfmt = '%.2f')
            self._ax_g3x.set_title("Gaussian position", fontsize='small')
            self._slider_g3x.on_changed(self.update)       
            
            self._ax_g3w = self.fig.add_axes([g3w_slider_x, g3w_slider_y, g3w_slider_w, g3w_slider_h], facecolor="khaki")
            self._slider_g3w = w.Slider(self._ax_g3w, "$w$", 0, 1.5, valinit = 0, valfmt = '%.2f')
            self._ax_g3w.set_title("Gaussian width", fontsize='small')
            self._slider_g3w.on_changed(self.update)
            
            self._ax_g3A = self.fig.add_axes([g3A_slider_x, g3A_slider_y, g3A_slider_w, g3A_slider_h], facecolor="khaki")
            self._slider_g3A = w.Slider(self._ax_g3A, "$A$", 0, 1, valinit = 0, valfmt = '%.2f')
            self._ax_g3A.set_title("Gaussian amplitude", fontsize='small')
            self._slider_g3A.on_changed(self.update)
            self.marker_gauss3, = self.ax.loglog(self.r[0],self.sigma[-1,0], marker='o', linestyle='None', markersize=10, color="khaki", markeredgecolor='black')
        ...
        pow_law_text = r'$10^{{{:.2f}}} \cdot r^{{{:.2f}}}$'.format(self._slider_Norm.val, self._slider_Pow.val)
        self.ax.set_title(pow_law_text+"$\\cdot (1 - $"+gauss1_text+"$-$"+gauss2_text+"$-$"+gauss3_text+"$)$")
        """
    ...
    def get_model(self):
        ...
        pow_law_text = r'$10^{{{:.2f}}} \cdot r^{{{:.2f}}}$'.format(self._slider_Norm.val, self._slider_Pow.val)
        #self.ax.set_title(pow_law_text+"$\\cdot (1 - $"+gauss1_text+"$-$"+gauss2_text+"$-$"+gauss3_text+"$)$")
        w.TextBox(self.axbox, '$model$', initial=pow_law_text+"$\\cdot (1 - $"+gauss1_text+"-"+gauss2_text+"-"+gauss3_text+")")
        """
        self.pow_law = 10**(self._slider_Norm.val)*self.r**(self._slider_Pow.val)
        self.gauss1, self.gauss2, self.gauss3 = 0, 0, 0
        if self.planet[0] == 1:
            self.gauss1 = (self._slider_g1A.val)*np.exp(-((self.r)-10**(self._slider_g1x.val))**2 / (2*(10**(self._slider_g1w.val))**2))
            
        if self.planet[1] == 1:
            self.gauss2 = (self._slider_g2A.val)*np.exp(-((self.r)-10**(self._slider_g2x.val))**2 / (2*(10**(self._slider_g2w.val))**2))
            
        if self.planet[2] == 1:
            self.gauss3 = (self._slider_g3A.val)*np.exp(-((self.r)-10**(self._slider_g3x.val))**2 / (2*(10**(self._slider_g3w.val))**2))
        return self.pow_law * (1 - self.gauss1 - self.gauss2 - self.gauss3)
    ...
    def update(self, event):
        ...
        if self.planet[0] == 1:
            ...
            self.marker_gauss1.set_ydata(10**(self._slider_Norm.val)*(10**self._slider_g1x.val)**(self._slider_Pow.val))
        ...
        if self.planet[1] == 1:
            ...
            self.marker_gauss2.set_label('$A_2 \\cdot e^{{-\\frac{{1}}{{2}}(\\frac{{r-r_0}}{{w_0}})^2}} = {{{:1.2g}}} \\cdot e^{{-\\frac{{1}}{{2}}(\\frac{{r-{{{:.2f}}}}}{{{{{:1.2g}}} }})^2}}$'.format(self._slider_g2A.val, self._slider_g2x.val, 10**self._slider_g2w.val))
            self.marker_gauss2.set_xdata(10**self._slider_g2x.val)
            self.marker_gauss2.set_ydata(10**(self._slider_Norm.val)*(10**self._slider_g2x.val)**(self._slider_Pow.val))
        ...
        if self.planet[2] == 1:
            ...
            self.marker_gauss3.set_ydata(10**(self._slider_Norm.val)*(10**self._slider_g3x.val)**(self._slider_Pow.val))
        ...
        self._ax_Norm.set_title("$y_0 = 10^{{{:.2f}}}$".format(self._slider_Norm.val, fontsize='small'))
        self._ax_Pow.set_title("$a = {{{:.2f}}}$".format(self._slider_Pow.val), fontsize='small')
        self.model_line_A.set_label(r'$ y_0 \cdot r^a = {{{:0.4g}}} \cdot r^{{{:.2g}}}$'.format(10**self._slider_Norm.val, self._slider_Pow.val))
...
def main():
    ...
    PARSER.add_argument('-n', '--number-planets', help='number of planets to model', type=int, choices=[0, 1, 2, 3])
